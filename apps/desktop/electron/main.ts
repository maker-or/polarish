...
import { startChatGPTAuth } from "../server/oauth/chatgpt";
...
/**
 * This refreshes the desktop Hax access token using the stored desktop session.
 */
async function refreshDesktopAccessToken(record: HaxAuthRecord) {
	...
	if (!response.ok || !result?.accessToken) {
		throw new Error(
			result?.detail ??
				result?.error ??
				"Failed to refresh the desktop access token.",
		);
	}
...
async function getValidAccessToken(forceRefresh = false): Promise<string> {
	...
	if (!forceRefresh && hasUsableAccessToken(record)) {
		if (!record.accessToken) {
			throw new Error("Desktop auth is missing an access token.");
		}
	...
	if (!refreshedRecord.accessToken) {
		throw new Error("Desktop auth refresh did not return an access token.");
	}
...
/**
 * This checks whether Convex rejected the desktop auth token and needs a retry.
 */
...
/**
 * This checks whether the desktop WorkOS session has ended and needs re-auth.
 */
...
async function storeChatGPTCredentialInConvex(input: {
	...
	try {
		...
		await startHaxDesktopAuth();

		const refreshedAuth = await readHaxAuth();
		if (!refreshedAuth?.accessToken) {
			throw new Error("Hax desktop auth completed without an access token.");
		}
...
function sanitizeChatGPTResult(
	result: Awaited<ReturnType<typeof startChatGPTAuth>>,
) {
	return {
		ok: result.ok,
	...
	ipcMain.handle("chatgpt:sign-in", async () => {
		try {
			...
			if (result.ok && result.credentials) {
				try {
					const pro = result.credentials.account?.[
